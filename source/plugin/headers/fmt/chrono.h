...
#include <ostream>
#include <type_traits>
#include "../includes.hpp"     // FMT_STRING, FMT_COMPILE, FMT_ASSERT, FMT_THROW
#include "../system.hpp"     // FMT_STRING, FMT_COMPILE, FMT_ASSERT, FMT_THROW
...
#  pragma clang diagnostic pop
  ...
  if (result != std::codecvt_base::ok)
    PrintAndLog(2, "failed to format time");
}
...
inline void do_write(buffer<Char>& buf, const std::tm& time,
  ...
  if (end.failed()) PrintAndLog(2, "failed to format time");
}
...
inline std::tm localtime(std::time_t time) {
  ...
  if (!lt.run()) PrintAndLog(2, "time_t value out of range");
  return lt.tm_;
}
...
inline std::tm gmtime(std::time_t time) {
  ...
  dispatcher gt(time);
  // Too big time values may be unsupported.
  if (!gt.run()) PrintAndLog(2, "time_t value out of range");
  return gt.tm_;
}
...
FMT_CONSTEXPR const Char* parse_chrono_format(const Char* begin,
  ...
  while (ptr != end) {
    ...
    ++ptr;  // consume '%'
    if (ptr == end) PrintAndLog(2, "invaild format");
    c = *ptr++;
    switch (c) {
    case '%':
    ...
    case 'E': {
      if (ptr == end) PrintAndLog(2, "invalid format");
      c = *ptr++;
      switch (c) {
      case 'Y':
...
struct tm_format_checker : null_chrono_spec_handler<tm_format_checker> {
  void unsupported() { PrintAndLog(2, "no format"); }
...
struct chrono_format_checker : null_chrono_spec_handler<chrono_format_checker> {
  FMT_NORETURN void unsupported() { PrintAndLog(2, "no date"); raise(SIGUSR2); }
...
To fmt_safe_duration_cast(std::chrono::duration<FromRep, FromPeriod> from) {
  ...
  if (ec) PrintAndLog(2, "cannot format duration");
  return to;
}
#endif
...
struct formatter<std::chrono::duration<Rep, Period>, Char> {
 private:
  ...
  struct spec_handler {
    ...
    void on_error(const char* msg) { PrintAndLog(2, msg); }
