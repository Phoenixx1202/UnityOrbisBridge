...
namespace UnityOrbisBridge
{
    ...
    public static partial class UOB
    {
        ...
        public static string GetDiskInfo(DiskInfo info)
        {
            var diskInfo = new Dictionary<DiskInfo, string>
            {
                { DiskInfo.Free, Marshal.PtrToStringAnsi(PrivateImports.GetDiskInfo("freeSpace")) },
                { DiskInfo.Used, Marshal.PtrToStringAnsi(PrivateImports.GetDiskInfo("usedSpace")) },
                { DiskInfo.Total, Marshal.PtrToStringAnsi(PrivateImports.GetDiskInfo("totalSpace")) },
                { DiskInfo.Percent, Marshal.PtrToStringAnsi(PrivateImports.GetDiskInfo("percentUsed")) }
            };
